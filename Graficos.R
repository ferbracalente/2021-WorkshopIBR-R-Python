...
datos$Cepa
datos$`%TIC` # Para acceder a la columna %TIC tengo que escaparla
...
#Exp4$Replicas<-factor(Exp4$Replicas, levels= Exp4Replicas)
#Exp4$Cepa<-factor(Exp4$Cepa,levels=Exp4Cepas)
...
p4 <- 
  ggplot(data = datos)+
  aes_string(x = "Comp", y = "ug", fill = "Replicas")+
  geom_bar(stat="identity", position=position_dodge(), width=0.75,
           color="black", linetype = "solid") +
  scale_x_discrete(limits = compuestos.SinEI)+ #Para que no cambie el orden de los compuestos
  theme_cowplot(line_size = 1) + # Fondo blanco
...
CompMetHidroxEnOrden <- c("2-mC12", "4-mC12", "8-mC12", "2-mC13",
...
graficarCompuestosFacetsPromedio(datosPromedio, CompMetHidroxEnOrden,
